...
export function CloudHeader({searchValue, onChangeSearchValue}: SearchTool) {
    ...
    const onClickToConfirmDel = () => {
        dispatch(deleteFolder({id: currentFolder!, previewFolder: [...curentfolders].splice(-2)[0].id}))
    }
    ...
    const onClickToConfirmEdit = (value: string) => {
        dispatch(editFolder({id: currentFolder!, folderName: value, currentFolder: currentFolder!, navNumber: curentfolders.length - 1}))
    }
    const onClick = () => {
